...
# rename data and prepare for normative modelling
#df.merge[[site]] = prepare.ukb.brainchart_link(df.out, site, codesite)
...
save(df.merge, 
     df.merge.long,
     #mod.stats,
     file = file.path(outdir, "df.all.cross.Rda"))
...
save(df.merge, 
     df.merge.long,
     #mod.stats.filt,
     file = file.path(outdir, "df.all.cross.filt.Rda"))
```
...
model_parameters = lapply(phenotypes.gamm, 
                          function(x) {import(file.path(outdir, model, x,"model_parameters.cross.csv"))})
...
write.csv(model_parameters, 
          quote = F, 
          row.names = F, 
          file = here('data-raw/tabulated/parameters_reliability/full.sample.cross.csv'))
```